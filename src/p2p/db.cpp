...
bool db::__open()
{
    ...
    if( !leveldb::DB::Open(opt, path_.c_str(), &pDB).ok() )
    {
        ...
        if( rc.ok() )
        {
            std::cout << "repaired db " << path_.c_str() << std::endl;
            rc = leveldb::DB::Open(opt, path_.c_str(), &pDB);
            if( !rc.ok() )
            {
                std::cout << "error open db " << path_.c_str() << std::endl;
                return false;
            }
        }
        else
        {
            std::cout << "error repair db " << path_.c_str() << std::endl;
            return false;
        }
    }
...
void db::commit()
{
    if(!db_.get() || !batch_.get())
        return;

    leveldb::WriteOptions op;
    leveldb::Status rc = db_->Write(op, batch_.get());
    if(!rc.ok())
        std::cout << "error commit" << std::endl;
    batch_.reset();
}
...
void db::__put(const char *key, size_t key_sz, const char *val, size_t val_sz)
{
    ...
    if(!batch_)
    {
        ...
        batch_->Put(slice_key, slice_val);
}
